...
function levelText(number) {
  var level = []
  level.push(randomizedLevelText(number))
  level.push(randomizedLevelText(number))
  level.push(randomizedLevelText(number))
  level.push(randomizedLevelText(number))
  return level//{1: [6 lines of text],2: 3: --6}
}
...
//
//
function randomizedLevelText(level){
  var wordLength = Math.round(rand(2,6));
  var wordIndex = Math.round(rand(0, levelWords[level][wordLength].length - 1))
  displayedWords.push(levelWords[level][wordLength][wordIndex]);
  var line = checkLineLength(displayedWords, level);
  return line
}
